...
# ---------------------------------------------------------
    ...
    st.subheader("👨‍⚕️ Treatment Workload Analysis")
...
# ---------------------------------------------------------
    ...
    st.subheader("🛏️ Service Capacity Analysis")
    st.caption(
        "Capacity metrics follow the methodology from the team's Service Capacity Analysis: "
        "bed occupancy, staffing workload, department utilization, critical-patient risk, "
        "monthly capacity trends, and hospital performance scorecard."
    )

    if not hospital_col or not beds_col or not patient_col:
        st.warning(
            "Service Capacity requires Hospital_Name, Total_Beds and Patient_ID columns."
        )
        st.stop()
    ...
    hospital_beds = (
        filtered.groupby(
            [hospital_col, hospital_type_col] if hospital_type_col else [hospital_col]
        )
        .agg(
            Patient_Count=(patient_col, "count"),
            Total_Beds=(beds_col, "first")
        )
        .reset_index()
    )
    ...
    hospital_beds["Occupancy_Rate"] = (
        ...
        hospital_beds["Total_Beds"].replace(0, pd.NA)
    ) * 100

    hospital_beds = hospital_beds.dropna(subset=["Occupancy_Rate"])
    hospital_beds = hospital_beds.sort_values("Occupancy_Rate", ascending=False)

    avg_occupancy = hospital_beds["Occupancy_Rate"].mean() if not hospital_beds.empty else 0
    over_85 = int((hospital_beds["Occupancy_Rate"] > 85).sum())
    under_70 = int((hospital_beds["Occupancy_Rate"] < 70).sum())

    c1, c2, c3 = st.columns(3)
    c1.metric("Average Occupancy", f"{avg_occupancy:.1f}%")
    c2.metric("Hospitals > 85%", f"{over_85}")
    c3.metric("Hospitals < 70%", f"{under_70}")
    ...
    if not hospital_beds.empty:
        fig = px.bar(
            ...
            labels={"Occupancy_Rate": "Occupancy %", hospital_col: "Hospital"}
        )
        ...
        st.info(
            "Hospitals operating above 85% occupancy are treated as critical-capacity "
            "facilities in the team's analysis; hospitals below 70% have comparatively "
    ...
    # 2. STAFFING WORKLOAD
    # -----------------------------------------------------
    st.subheader("👨‍⚕️ Staffing Workload")

    if dept_col and doctor_id_col and nurse_id_col:
        staffing = (
            filtered.groupby([hospital_col, dept_col])
            .agg(
                Patient_Count=(patient_col, "count"),
                Doctors=(doctor_id_col, "nunique"),
                Nurses=(nurse_id_col, "nunique")
            )
            .reset_index()
        )
        ...
        staffing["Patients_per_Doctor"] = (
            ...
            staffing["Doctors"].replace(0, pd.NA)
        )
        staffing["Patients_per_Nurse"] = (
            staffing["Patient_Count"] /
            staffing["Nurses"].replace(0, pd.NA)
        )

        staffing = staffing.sort_values(
        ...
        st.info(
            "The source analysis flags departments above 15 patients per doctor "
            "or above 10 patients per nurse for staffing attention."
        )

    elif dept_col and doctor_id_col:
        st.warning(
            "Nurse_ID is not available, so only the patient-to-doctor workload is shown."
        )
        ...
        staffing = staffing.sort_values("Patient_Count", ascending=False).head(10)
        ...
        fig.add_hline(y=15, line_dash="dash", annotation_text="Doctor reference (15:1)")
        fig.update_layout(xaxis_tickangle=-45)
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.warning("Doctor_ID and/or Nurse_ID columns are not available.")

    # -----------------------------------------------------
    # 3. DEPARTMENT-WISE UTILIZATION HEATMAP
    # -----------------------------------------------------
    if dept_col and not hospital_beds.empty:
        st.subheader(" Department-Wise Bed Utilization")
        ...
        dept_util["Utilization"] = (
            ...
            dept_util["Total_Beds"].replace(0, pd.NA)
        ) * 100

        heatmap_data = dept_util.pivot_table(
            values="Utilization",
            index=hospital_col,
            columns=dept_col,
            fill_value=0
    ...
    # -----------------------------------------------------
    # 4. CRITICAL PATIENT CAPACITY RISK
    # -----------------------------------------------------
    if severity_col:
        st.subheader("🚨 Critical Patient Capacity Risk")

        critical_levels = ["Critical", "High", "Medium"]
        critical_data = filtered[
            filtered[severity_col].isin(critical_levels)
        ].copy()

        critical_summary = (
            critical_data.groupby([hospital_col, severity_col])
            .agg(Patient_Count=(patient_col, "count"))
            .reset_index()
        )

        critical_summary = critical_summary.pivot_table(
        ...
        bed_info = (
            filtered.groupby(hospital_col)[beds_col]
            .first()
            .reset_index()
        )

        critical_summary = critical_summary.merge(
            bed_info, on=hospital_col, how="left"
        )
        ...
        critical_summary["Risk_Score"] = (
            critical_summary["Total_Critical"] /
            critical_summary[beds_col].replace(0, pd.NA)
        ) * 100

        critical_summary = (
            critical_summary.dropna(subset=["Risk_Score"])
        ...
        if not critical_summary.empty:
            fig = px.bar(
                ...
                labels={"Risk_Score": "Critical Patient Risk %"}
            )
            fig.update_traces(texttemplate="%{text:.1f}%", textposition="outside")
    ...
    # -----------------------------------------------------
    # 5. MONTHLY CAPACITY TREND
    # -----------------------------------------------------
    if date_col:
        st.subheader("📅 Monthly Capacity Trend")

        monthly = filtered.dropna(subset=[date_col]).copy()
        monthly["Month"] = monthly[date_col].dt.to_period("M").dt.to_timestamp()

        monthly_trend = (
            monthly.groupby(["Month", hospital_col])
            .agg(
                Patient_Count=(patient_col, "count"),
                Total_Beds=(beds_col, "first")
            )
        ...
        monthly_trend["Occupancy"] = (
            ...
            monthly_trend["Total_Beds"].replace(0, pd.NA)
        ) * 100
    ...
    # -----------------------------------------------------
    # 6. HOSPITAL PERFORMANCE SCORECARD
    # -----------------------------------------------------
    if not hospital_beds.empty:
        st.subheader("🏆 Hospital Performance Scorecard")

        score_df = hospital_beds.copy()
        score_df["Score"] = score_df["Occupancy_Rate"].apply(
            lambda x: "A (Good)" if x < 70
            else "B (Moderate)" if x < 85
            else "C (Critical)"
        )

        scorecard = (
            score_df.groupby("Score")[hospital_col]
            .count()
        ...
        # Dynamic capacity insight
        critical_hospitals = score_df[
            score_df["Score"] == "C (Critical)"
        ][hospital_col].tolist()

        if critical_hospitals:
            st.warning(
                "Critical-capacity hospitals: " +
                ", ".join(map(str, critical_hospitals[:10]))
            )
        else:
            st.success("No hospital is above the critical 85% occupancy threshold in the current filter.")
